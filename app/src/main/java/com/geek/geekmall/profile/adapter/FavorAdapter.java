...
public class FavorAdapter extends BaseAdapter {
    ...
    public FavorAdapter(Context context,PullToRefreshGridView gridView) {
        mContext = context;
        mGridView = gridView;
    }

    public void setmProducts(List<FavorProduct> mProducts) {
        this.mProducts = mProducts;
    }

    public List<FavorProduct> getmProducts() {
        return mProducts;
    }
    ...
    @Override
    public FavorProduct getItem(int i) {
        if (mProducts == null) {
            return null;
        }
...
//            ImageLoader.getInstance(mContext).getUniversalImageLoader().displayImage(url, viewHolder.cover, options);
        ...
        ((LikeActivity)mContext).updateButton();
        MyLog.debug(FavorAdapter.class,"------"+position+"---------");
        return convertView;
    }
